...
export function RecipientAutosuggest({
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return validatedSuggestions.filter((suggestion) => !recipients.includes(suggestion.email));
  ...
  return (
    <div className={cn('relative w-full', className)}>
      ...
      {isOpen && (filteredSuggestions.length > 0 || isLoading) && (
        <div
          ref={dropdownRef}
          className="bg-popover animate-in fade-in-0 zoom-in-95 absolute left-0 right-0 top-full z-50 mt-1 max-h-60 overflow-auto rounded-md border p-1 shadow-md"
