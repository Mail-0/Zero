...
>(({ className, ...props }, ref) => (
  <div className="border-input flex w-full items-center border-none px-5" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        'placeholder:text-muted-foreground/70 outline-hidden flex h-10 w-full rounded-lg bg-transparent py-3 text-sm disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      'data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground outline-hidden relative flex cursor-pointer select-none items-center gap-4 rounded-md px-2 py-1.5 text-sm transition-colors data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
      className,
    )}
    {...props}
  />
