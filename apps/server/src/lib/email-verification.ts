...
const resolveTxtSafe = (hostname: string, timeout = DNS_TIMEOUT_MS): Promise<string[][]> => {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error(`DNS resolveTxt timeout for ${hostname}`)),
      timeout,
    );
    dns
      .resolveTxt(hostname)
...
async function validateDKIM(rawEmail: string): Promise<boolean> {
  try {
    ...
    const canonicalizedHeaders = headerList
      .map((headerName) => {
...
async function validateBIMI(domain: string): Promise<boolean> {
  try {
    ...
    try {
      ...
      console.log(
        `[BIMI_DEBUG] No BIMI record on exact domain ${domain}: ${exactDomainError instanceof Error ? exactDomainError.message : String(exactDomainError)}`,
      );
    }
    ...
    if (domainParts.length > 2) {
      ...
      try {
        ...
        const parentBimiRecord = parentTxtRecords
          .flat()
          .find((record) => record.includes('v=BIMI1'));
        ...
        console.log(
          `[BIMI_DEBUG] Error checking parent domain ${parentDomain}: ${parentDomainError instanceof Error ? parentDomainError.message : String(parentDomainError)}`,
        );
      }
    } else {
...
async function validateBIMIRecord(bimiRecord: string, domain: string): Promise<boolean> {
  try {
    ...
    console.log(
      `[BIMI_DEBUG] Validating BIMI record for ${domain}: logoUrl=${logoUrl}, vmcUrl=${vmcUrl}`,
    );
    ...
    console.log(
      `[BIMI_DEBUG] Error validating BIMI record for ${domain}:`,
...
async function validateVMC(vmcUrl: string, domain: string): Promise<boolean> {
  try {
    ...
    console.log(
      `[BIMI_DEBUG] VMC validation error for ${domain}:`,
      error instanceof Error ? error.message : String(error),
    );
    return false;
  }
...
async function validateLogo(logoUrl: string, domain: string): Promise<boolean> {
  try {
    ...
    console.log(
      `[BIMI_DEBUG] Logo validation error for ${domain}:`,
      error instanceof Error ? error.message : String(error),
    );
    return false;
  }
...
export async function verify(rawEmail: string): Promise<{ isVerified: boolean; logoUrl?: string }> {
  try {
    ...
    const [spfValid, dkimValid, dmarcValid, bimiValid] = await Promise.all([
      senderIP
        ? validateSPF(domain, senderIP).catch((_error) => {
            return false;
          })
    ...
    console.log(
      `[VERIFY_DEBUG] Domain: ${domain}, SPF: ${spfValid}, DKIM: ${dkimValid}, DMARC: ${dmarcValid}, BIMI: ${bimiValid}`,
    );
    const isVerified = authValid && bimiValid;
    console.log(
      `[VERIFY_DEBUG] Final verification result for ${domain}: ${isVerified} (auth: ${authValid}, bimi: ${bimiValid})`,
    );
