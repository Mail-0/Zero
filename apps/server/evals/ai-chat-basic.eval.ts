...
evalite('AI Chat – Smart Categorization', {
  data: makeAiChatTestCaseBuilder(
    'smart categorization (subscriptions, newsletters, meetings, bills)',
  ),
  task: async (input) => {
    return safeStreamText({
      model: model,
...
evalite('AI Chat – Information Queries', {
  data: makeAiChatTestCaseBuilder(
    'information queries (summaries, web search, tax docs, recent activity)',
  ),
  task: async (input) => {
    return safeStreamText({
      model: model,
...
evalite('AI Chat – Error Handling & Edge Cases', {
  data: makeAiChatTestCaseBuilder(
    'error handling & edge cases (invalid, bulk actions, very old queries)',
  ),
  task: async (input) => {
    return safeStreamText({
      model: model,
